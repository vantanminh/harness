...
| Project-authored tool probe executes attacker code | `tool check` requires the same explicit approval and bounded command shape; MCP does not expose tool-check execution | An approved probe still has the local OS user's permissions; use a trusted project script or external sandbox |
