...
| Public dashboard exposes project data | Loopback default; non-loopback requires valid HTTPS URL and modern Argon2id password; missing credentials fail closed at request time; 30 failed attempts/minute/source throttle with `Retry-After`; security headers and no-store responses | Reverse-proxy/TLS configuration is operator responsibility |
